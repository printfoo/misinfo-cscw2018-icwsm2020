...
if __name__ == "__main__":
    ...
    fig_path = os.path.join(file_path, "results", "ratings_q2.pdf")
    ...
    # plot fig1
    handle = ax[0].violinplot([df["rating_2"] for df in dfs],
                              showmeans = True, showextrema = False, widths = 0.7)
    print([df["rating_2"].mean() for df in dfs])
    ...
    ax[0].set_yticklabels([r"$\bf{Not\ accurate}$ (1)", r"$\bf{Slightly\ accurate}$ (2)",
                           r"$\bf{Moderately\ accurate}$ (3)", r"$\bf{Very\ accurate}$ (4)", r"$\bf{Extremely\ accurate}$ (5)"])
    ...
    for i in range(len(["shan", "dan", "jun", "ming"])):
        for j in range(len(["shan", "dan", "jun", "ming"])):
            m[i][j], p = pearsonr(dfs[i]["rating_2"], dfs[j]["rating_2"])
            m[i][j] = round(m[i][j], 3)
            print(m[i][j], p)
    print(kruskal(dfs[0]["rating_2"], dfs[1]["rating_2"], dfs[2]["rating_2"], dfs[3]["rating_2"]))
    map = ax[1].imshow(m, cmap = "Purples", alpha = 0.8, vmin = 0, vmax = 1)
